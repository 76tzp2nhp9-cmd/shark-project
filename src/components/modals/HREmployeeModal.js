...
const HREmployeeModal = ({ onClose, onSubmit, employee = null, isEdit = false }) => {
  const [formData, setFormData] = useState({
    agent_name: '', designation: 'Agent', cnic: '', joining_date: new Date().toISOString().split('T')[0],
    bank_name: '', account_number: '', status: 'Active'
  });

  // [FIX] Populate fields with existing employee data
  useEffect(() => {
    if (isEdit && employee) {
      setFormData({
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-xl shadow-xl p-6 w-[500px] max-h-[90vh] overflow-y-auto">
        <h3 className="text-xl font-semibold text-slate-800 mb-4 flex items-center gap-2"><Briefcase className="w-5 h-5 text-blue-600"/> {isEdit ? 'Edit Record' : 'New Employment Record'}</h3>
        ...
        <div className="flex gap-3 mt-6">
          <button onClick={() => onSubmit(formData)} className="flex-1 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700">{isEdit ? 'Update Record' : 'Save Record'}</button>
